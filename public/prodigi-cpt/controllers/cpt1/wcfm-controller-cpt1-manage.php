	  ...
	  if(isset($wcfm_cpt1_manage_form_data['title']) && !empty($wcfm_cpt1_manage_form_data['title'])) {
	  	...
	  	// Creating new cpt1
			...
			if(!is_wp_error($new_cpt1_id)) {
				...
				// Custom Fields 
				if(isset($wcfm_cpt1_manage_form_data['trifie_sku_recomended_size']) && !empty($wcfm_cpt1_manage_form_data['trifie_sku_recomended_size'])) {
					update_post_meta( $new_cpt1_id, 'trifie_sku_recomended_size', $wcfm_cpt1_manage_form_data['trifie_sku_recomended_size'] );
				} else {
					update_post_meta( $new_cpt1_id, 'trifie_sku_recomended_size', '' );
				}
				if(isset($wcfm_cpt1_manage_form_data['prodigi_trifie_sku']) && !empty($wcfm_cpt1_manage_form_data['prodigi_trifie_sku'])) {
					update_post_meta( $new_cpt1_id, 'prodigi_trifie_sku', $wcfm_cpt1_manage_form_data['prodigi_trifie_sku'] );
				} else {
					update_post_meta( $new_cpt1_id, 'prodigi_trifie_sku', '' );
				}
				if(isset($wcfm_cpt1_manage_form_data['trifie_product_id']) && !empty($wcfm_cpt1_manage_form_data['trifie_product_id'])) {
					update_post_meta( $new_cpt1_id, 'trifie_product_id', $wcfm_cpt1_manage_form_data['trifie_product_id'] );
				} else {
					update_post_meta( $new_cpt1_id, 'trifie_product_id', '' );
				}
				if(isset($wcfm_cpt1_manage_form_data['trifie_product_cost']) && !empty($wcfm_cpt1_manage_form_data['trifie_product_cost'])) {
					update_post_meta( $new_cpt1_id, 'trifie_product_cost', $wcfm_cpt1_manage_form_data['trifie_product_cost'] );
				} else {
					update_post_meta( $new_cpt1_id, 'trifie_product_cost', '' );
				}
				if(isset($wcfm_cpt1_manage_form_data['trifie_product_min_price']) && !empty($wcfm_cpt1_manage_form_data['trifie_product_min_price'])) {
					update_post_meta( $new_cpt1_id, 'trifie_product_min_price', $wcfm_cpt1_manage_form_data['trifie_product_min_price'] );
				} else {
					update_post_meta( $new_cpt1_id, 'trifie_product_min_price', '' );
				}
